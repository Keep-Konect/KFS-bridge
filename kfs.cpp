...
namespace FileSystem {
    ...
    void KFS::InitKFS() {
        ...
        uint64_t MemTotPartiton = (globalPartition->partition->LastLBA - globalPartition->partition->FirstLBA) * 512;
        ...
        info->Root.FID = 0;
        char* Name = (char*)"Kot file system";
    ...
    AllocatePartition* KFS::Allocate(uint64_t size, Folder* folder, uint64_t LastClusterRequested, bool GetAutoLastCluster) {
    ...
    void KFS::flist(char* filePath) {
        char** FoldersSlit = split(filePath, (char*)"/");
        int count = 0;
            ...
            for (int i = 0; i <= count; i++) {
                while (true) {
                    ...
                    if (!ScanHeader->IsFile && ScanHeader->FID != 0) {
                        ...
                        if (strcmpKot(folderInfo->Name, FoldersSlit[i])) {
    ...
    uint64_t KFS::mkdir(char* filePath, uint64_t Mode) {
        ...
        time_t now = time(0);
        tm* gmtm = gmtime(&now);
        folderInfo->TimeInfoFS.CreateTime.seconds = gmtm->tm_sec;
        folderInfo->TimeInfoFS.CreateTime.minutes = gmtm->tm_min;
        folderInfo->TimeInfoFS.CreateTime.hours = gmtm->tm_hour;
        folderInfo->TimeInfoFS.CreateTime.days = gmtm->tm_mday;
        folderInfo->TimeInfoFS.CreateTime.months = gmtm->tm_mon;
        folderInfo->TimeInfoFS.CreateTime.years = gmtm->tm_year + 1900;
        ...
        for (int i = 0; i <= count; i++) {
            while (true) {
                ...
                if (ScanHeader->IsFile) {
                    ...
                    if (i == count && strcmpKot(FileName, FoldersSlit[count])) {
                        returnData = (File*)malloc(sizeof(File));
                    ...
                    if (strcmpKot(FileName, FoldersSlit[i])) {
                        ScanCluster = folderInfo->FirstClusterData;
                        if (folder != NULL) {
    ...
    FileInfo* KFS::NewFile(char* filePath, Folder* folder) {
        ...
        time_t now = time(0);
        tm* gmtm = gmtime(&now);
        fileInfo->TimeInfoFS.CreateTime.seconds = gmtm->tm_sec;
        fileInfo->TimeInfoFS.CreateTime.minutes = gmtm->tm_min;
        fileInfo->TimeInfoFS.CreateTime.hours = gmtm->tm_hour;
        fileInfo->TimeInfoFS.CreateTime.days = gmtm->tm_mday;
        fileInfo->TimeInfoFS.CreateTime.months = gmtm->tm_mon;
        fileInfo->TimeInfoFS.CreateTime.years = gmtm->tm_year + 1900;
    ...
    bool KFS::UpdatePartitionInfo() {
        globalPartition->Write(0, sizeof(KFSinfo), KFSPartitionInfo);
        return true;
    }
    ...
    uint64_t File::Read(uint64_t start, uint64_t size, void* buffer) {
        ...
        while (bytesRead < size) {
            ...
            if (bytesRead != 0) {
                memcpy((void*)((uint64_t)buffer + bytesRead), (void*)((uint64_t)Cluster + sizeof(ClusterHeader)), bytesToRead);
            }
            else {
                if (ReadCluster == fileInfo->ClusterHeaderPostition) {
    ...
    uint64_t File::Write(uint64_t start, uint64_t size, void* buffer) {
        //let's check if we need to enlarge the file or shrink it
        void* Cluster = malloc(kfs->KFSPartitionInfo->ClusterSize);
        memset(Cluster, 0, kfs->KFSPartitionInfo->ClusterSize);
        ...
        if (ClusterTotal != fileInfo->ClusterSize && ClusterTotal != 0) {
            if (ClusterTotal > fileInfo->ClusterSize) {
                uint64_t NewSize = (ClusterTotal - fileInfo->ClusterSize) * kfs->KFSPartitionInfo->ClusterSize - sizeof(ClusterHeader);
                //Alloc new Clusters
                ...
                uint64_t NewSize = ClusterTotal * kfs->KFSPartitionInfo->ClusterSize - sizeof(ClusterHeader);
                //Free last Cluster
        ...
        while (bytesWrite < size) {
            ...
            if (bytesWrite != 0) {
                memcpy((void*)((uint64_t)Cluster + sizeof(ClusterHeader)), (void*)((uint64_t)buffer + bytesWrite), bytesToWrite);
            }
            else {
                if (WriteCluster == fileInfo->ClusterHeaderPostition) {
                    FirstByte += sizeof(HeaderInfo) + sizeof(FileInfo);
